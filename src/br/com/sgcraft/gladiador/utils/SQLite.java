...
public class SQLite {
	...
	public SQLite() {
		try {
			...
			this.connection = DriverManager.getConnection(
					"jdbc:sqlite:" + SQLite.pl.getDataFolder().getAbsolutePath() + File.separator + "database.db");
			(this.stmt = this.connection.createStatement())
					.execute("CREATE TABLE IF NOT EXISTS topClans (clan VARCHAR(255), clanTag VARCHAR(255), wins INTEGER)");
			(this.stmt = this.connection.createStatement())
					.execute("CREATE TABLE IF NOT EXISTS topKills (player VARCHAR(255), kills INTEGER)");
		...
		try {
			...
			while (rs.next()) {
				if (++i == 0) {
					...
					p.sendMessage(SQLite.pl.getConfig().getString("Top_Vencedores_Posicao").replace("&", "§")
							.replace("@posicao", new StringBuilder(String.valueOf(i)).toString())
							.replace("@clan", rs.getString("clanTag"))
							.replace("@vitorias", new StringBuilder(String.valueOf(rs.getInt("wins"))).toString()));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void updateKills(final String player, final int kills) {
		try {
			Class.forName("org.sqlite.JDBC");
			final String sql = "UPDATE topKills SET kills='" + (this.getKills(player) + kills) + "' WHERE player='"
					+ player + "';";
			this.stmt.executeUpdate(sql);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void getTOPKills(final Player p) {
		try {
			...
			while (rs.next()) {
				if (++i == 0) {
					p.sendMessage("§6[Gladiador] §cNenhum player no TopKills!");
				} else {
					p.sendMessage(SQLite.pl.getConfig().getString("Top_Kills_Posicao").replace("&", "§")
							.replace("@posicao", new StringBuilder(String.valueOf(i)).toString())
							.replace("@player", rs.getString("player").toUpperCase())
							.replace("@kills", new StringBuilder(String.valueOf(rs.getInt("kills"))).toString()));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
