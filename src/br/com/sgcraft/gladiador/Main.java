...
public class Main extends JavaPlugin
{
    ...
    public void onDisable() {
        this.getLogger().info("Desativando Gladiador - Autor: SubZero0");
    }
	...
	public void betting(final Player p, final Clan clan) {
        ...
        this.apostadores.add(String.valueOf(String.valueOf(String.valueOf(p.getName()))) + ";" + clan.getTag());
    ...
    private void messageIniciando(final int vezes) {
        ...
        if (vezes == 0) {
            ...
            for (final String n : Mensagens.getIniciando()) {
                ...
                this.getServer().dispatchCommand((CommandSender)this.getServer().getConsoleSender(), "title &6&lGLADIADOR<nl>&bPrepare-se" );
            }
    ...
    protected void startGladiador() {
        if (this.getClansParticipandoSize() < 2) {
            ...
            for (final String n : Mensagens.getCancelar()) {
                this.getServer().broadcastMessage(n.replace("&", "§"));
                this.getServer().dispatchCommand((CommandSender)this.getServer().getConsoleSender(), "title &6&lGLADIADOR<nl>&cEvento Cancelado!" );
            }
            return;
        }
        this.canStart = true;
	...
	protected void checkGladiadorEnd_core1() {
        ...
        if (lista.size() == 1 && this.gladiadorEtapa == 3) {
            ...
            for (final String n : this.totalParticipantes.keySet()) {
                if (this.core1.getClanManager().getClan(vencedor.toString()) != null ){
            ...
            for (final String n : Mensagens.getFinalizar()) {
            	this.getServer().dispatchCommand((CommandSender)this.getServer().getConsoleSender(), "title &6&lGLADIADOR<nl>&7Parabéns "+ vencedor.getColorTag() );
                if ((this.mito || !n.startsWith("(mito)")) && (!this.mito || !n.startsWith("(gladiador)"))) {
                    if (n.startsWith("(gladiador)")) {
                        this.getServer().broadcastMessage(n.replace("&", "§").substring(11).replace("@clan", vencedor.getColorTag()).replace("@dinheiro", Double.toString(this.getConfig().getDouble("Premios.Dinheiro"))));
                    }
                    else if (n.startsWith("(mito)")) {
                        this.getServer().broadcastMessage(n.replace("&", "§").substring(6).replace("@clan", vencedor.getColorTag()).replace("@dinheiro", Double.toString(this.getConfig().getDouble("Premios.Dinheiro"))).replace("@mito", String.valueOf(String.valueOf(String.valueOf(v1))) + " (" + v1_v + ")"));
                    }
                    else if (!this.mito) {
                        this.getServer().broadcastMessage(n.replace("&", "§").replace("@clan", vencedor.getColorTag()).replace("@dinheiro", Double.toString(this.getConfig().getDouble("Premios.Dinheiro"))));
                    }
                    else {
                        this.getServer().broadcastMessage(n.replace("&", "§").replace("@clan", vencedor.getColorTag()).replace("@dinheiro", Double.toString(this.getConfig().getDouble("Premios.Dinheiro"))).replace("@mito", String.valueOf(String.valueOf(String.valueOf(v1))) + " (" + v1_v + ")"));
                    }
                ...
                for (int i = 0; i < ctag.length(); ++i) {
                    ...
                    if (Character.compare(Character.toLowerCase(c), Character.toLowerCase(ntag.charAt(parte))) == 0 && !this.lastchar(ctag, i)) {
                        if (lastcor.equals(ChatColor.getLastColors(ctag.substring(0, i)))) {
                            final_tag = String.valueOf(String.valueOf(String.valueOf(final_tag))) + c;
                        }
                        else {
                            final_tag = String.valueOf(String.valueOf(String.valueOf(final_tag))) + ChatColor.getLastColors(ctag.substring(0, i)) + c;
        ...
        for (int j = 0; j < ctag2.length(); ++j) {
            ...
            if (Character.compare(Character.toLowerCase(c2), Character.toLowerCase(ntag2.charAt(parte2))) == 0 && !this.lastchar(ctag2, j)) {
                if (lastcor2.equals(ChatColor.getLastColors(ctag2.substring(0, j)))) {
                    final_tag2 = String.valueOf(String.valueOf(String.valueOf(final_tag2))) + c2;
                }
                else {
                    final_tag2 = String.valueOf(String.valueOf(String.valueOf(final_tag2))) + ChatColor.getLastColors(ctag2.substring(0, j)) + c2;
            ...
            public void run() {
                ...
                for (final String xx : Main.this.clann.keySet()) {
                    if (Main.this.clann.get(xx) != 0 && Main.this.getClansParticipandoSize() > 1) {
                        sb.append(String.valueOf(String.valueOf(String.valueOf(xx.replace("[", "").replace("]", "")))) + " §e[" + Main.this.clann.get(xx) + "], ");
                    }
                }
