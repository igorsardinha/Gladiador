...
public class Listeners implements Listener
{
    ...
    private void onDamageP(final PotionSplashEvent e) {
        for (final Entity ent2 : e.getAffectedEntities()) {
            if (ent2 instanceof Player && this.plugin.getGladiadorEtapa() != 0) {
                ...
                if (dam == null || this.plugin.getGladiadorEtapa() != 3 || !this.plugin.participantes.contains(ent3.getName())) {
                    continue;
                }
    ...
    private void onPCmd(final PlayerCommandPreprocessEvent e) {
        ...
        if (this.plugin.getGladiadorEtapa() == 3 && e.getMessage().toLowerCase().startsWith("/clan") && !e.getMessage().toLowerCase().startsWith("/clan resign") && !e.getMessage().toLowerCase().startsWith("/clan kick") && !e.getMessage().toLowerCase().startsWith("/clan invite") && !e.getMessage().toLowerCase().startsWith("/clan trust") && !e.getMessage().toLowerCase().startsWith("/clan profile") && e.getMessage().toLowerCase().startsWith("/x9") && e.getMessage().toLowerCase().startsWith("/sgcraft:x9")) {
